...
const Turn =({author, books, highlight, onAnswerSelected})=>{
  const highlightedToBg = (highlight)=>{
    const mapping={
      none:'',
      correct:'green',
      wrong: 'red'
  ...
  return(
    <div className="row turn" style={{backgroundColor: highlightedToBg(highlight)}} >
...
({turnData, highlight, onAnswerSelected, onContinue})=>{
  return(
    <div className="container-fluid">
      ...
      <p className="offset-5"> <Link to="/add">Add an Author</Link> </p>
      <Footer />
    </div>
  );
});
